...
class GnuPG
{
     ...
     * @return bool
     */
    private function forkProcess($command, $input = false, &$output = null)
    {
    ...
    public function import($keyBlock)
    {
        ...
        if ($this->forkProcess(
            ...
            foreach ($contents as $data) {
                $matches = false;
                if (preg_match('/\[GNUPG:\]\sIMPORT_OK\s(\w+)\s(.+)/', $data, $matches)) {
     ...
     * @return bool|array false on error, the fingerprint of the created key pair in success
     */
    ...
    public function encrypt($keyId, $passPhrase, $recipientKeyId, $text, $sign = true)
    {
        ...
        $result = $this->forkProcess(
            $this->buildGnuPGCommand($options),
            $passPhrase."\n".$text,
            $contents
        );
